...
function get_promote_goods_wap($cats = '')
{
    ...
    /* 取得促销lbi的数量限制 */
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price,g.goods_weight, ' .
    ...
    foreach ($result AS $idx => $row)
    {
        ...
        $goods[$idx]['id']           = $row['goods_id'];
		 $goods[$idx]['goods_weight']           = $row['goods_weight']*1000;
 ...
 */
function cart_goods_list($type = CART_GENERAL_GOODS,$is_str)
{
    $sql = "SELECT a.rec_id, a.user_id, a.goods_id, a.goods_name,b.promote_price,b.promote_start_date,b.promote_end_date,a.goods_sn, a.goods_number, " .
            ...
            " b WHERE b.goods_id=a.goods_id and a.user_id = '" .  $_SESSION['user_id']  . "' " .
            "AND a.rec_type = '$type'";
	//if($is_str){
	if($is_str=="09123123" || $is_str==",09123123" || $is_str=="") {
		 
	 
	}else{
		$sql.=" AND a.rec_id in (".$_SESSION["rec_id_str"].")";
	}
    ...
    foreach ($arr as $key => $value)
    {
		...
		$value['promote_price']=$promote_price;
				
        $arr[$key]['formated_promote_price'] = price_format($value['promote_price'], false);
 ...
 * @return  array   购物车商品数组
 */
function cart_goodspp($type = CART_GENERAL_GOODS)
{
...
function get_recommend_goods_wap($type = '', $num, $cats = '')
{
    ...
    if (empty($type_goods[$type]))
    {
        ...
        if ($data === false)
        {
            $sql = 'SELECT g.goods_id, g.is_best, g.is_new, g.is_hot, g.is_promote, b.brand_name,g.sort_order,g.goods_weight ' .
               ' FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
               ' LEFT JOIN ' . $GLOBALS['ecs']->table('brand') . ' AS b ON b.brand_id = g.brand_id ' .
        ...
        foreach ($result AS $idx => $row)
        {
            ...
            $goods[$idx]['name']         = $row['goods_name'];
            $goods[$idx]['brief']        = $row['goods_brief'];
			$goods[$idx]['goods_weight']        = $row['goods_weight'];
 ...
 * @return  array   购物车商品数组
 */
function cart_goods_mobile($type = CART_GENERAL_GOODS,$is_str="")
{
 ...
 $sql = "SELECT a.rec_id, a.user_id, a.goods_id, a.goods_name,b.promote_price,b.promote_start_date,b.promote_end_date,a.goods_sn, a.goods_number, " .
	...
	if($is_str=="09123123" || $is_str==",09123123" || $is_str=="") {
		...
		$sql.=" AND rec_id in (".$_SESSION["rec_id_str"].")";
	}
	// echo $sql;
    $arr = $GLOBALS['db']->getAll($sql);
 ...
 * @param   int     $type   类型：默认普通商品
 */
function clear_cart_mobile($type = CART_GENERAL_GOODS,$is_str="")
{
